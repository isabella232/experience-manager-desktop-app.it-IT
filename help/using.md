...
Quando aprite l&#39;applicazione dopo la disconnessione o per la prima volta, fornite l&#39;URL del server di Experience Manager  nel formato `https://[aem-server-url]:[port]/`. Selezionare quindi l&#39;opzione [!UICONTROL Connect]. Fornite le credenziali per collegare l&#39;app al server.

Le attività principali che eseguite utilizzando l&#39;app desktop  Experience Manager sono:

![Flussi di lavoro e attività che puoi eseguire utilizzando l’ [!DNL Experience Manager] app ](assets/aem_desktop_app_usecases_v2.png "desktopFlussi di lavoro e attività che puoi eseguire utilizzando l’")
app desktop Adobe Experience ManagerScarica  [](assets/aem_desktop_app_usecases_print.pdf) questo file PDF pronto per la stampa.
...
* **[!UICONTROL Open In Web]** azione: Per visualizzare la risorsa nell’interfaccia Web  Experience Manager, apritela nel Web. Puoi avviare più flussi di lavoro &#39;interfaccia del Experience Manager, come l&#39;aggiornamento dei metadati o l&#39;individuazione delle risorse.

* **[!UICONTROL Delete]** azione: Eliminate la risorsa dall&#39;archivio DAM del Experience Manager . L’azione elimina la copia originale della risorsa sul server del Experience Manager . Se desiderate eliminare solo le modifiche apportate alla risorsa locale, consultate [eliminare le modifiche](using.md#edit-assets-upload-updated-assets).

* **[!UICONTROL Upload Changes]**: L&#39;app desktop carica la risorsa aggiornata solo quando viene caricata in modo esplicito  server di Experience Manager. Quando salvate le modifiche, queste vengono salvate solo sul computer locale. Quando caricate, la risorsa viene automaticamente archiviata e l&#39;icona Blocca viene rimossa. Consultate [modificare le risorse](using.md#edit-assets-upload-updated-assets).

## Abilitare le azioni desktop nell&#39;interfaccia Web  Experience Manager {#desktopactions-v2}
...
Potete cercare, cercare e visualizzare in anteprima le risorse disponibili nell’archivio del Experience Manager , il tutto dall’applicazione desktop. Provate quanto segue nell&#39;app:
...
1. Effettuate ricerche utilizzando le parole chiave per trovare una risorsa correlata nell’archivio del Experience Manager . Utilizzate `?` e `*` come caratteri jolly. Questi caratteri jolly sostituiscono rispettivamente un singolo carattere o più caratteri. Filtrate e ordinate i risultati come necessario.
...
Potete scaricare le risorse dal file system locale. L&#39;app recupera le risorse  server di Experience Manager e salva la stessa copia nel file system locale.
...
Se le cartelle sono selezionate e scaricate, l’applicazione scarica solo le risorse memorizzate direttamente nelle cartelle  Experience Manager. Non scarica automaticamente le risorse dalle sottocartelle.
...
## Modificare le risorse e caricare le risorse aggiornate sul Experience Manager  {#edit-assets-upload-updated-assets}
...
Dopo aver apportato le modifiche, l&#39;app visualizza lo stato **[!UICONTROL Edited Locally]** per le risorse modificate. Tutte le modifiche salvate nelle risorse sono locali solo fino a quando non caricate le modifiche  Experience Manager. Per caricare una o più risorse una per una, fate clic su **[!UICONTROL Upload Changes]** tra le opzioni per una risorsa. Crea una versione della risorsa in  Experience Manager. Utilizzando l&#39;interfaccia Web di [!DNL Assets], è possibile visualizzare la cronologia delle risorse nella [visualizzazione Timeline](https://experienceleague.adobe.com/docs/experience-manager-65/assets/using/activity-stream.html).
...
* Se non si desidera salvare le modifiche locali in  Experience Manager.
...
## Caricare e aggiungere nuove risorse al Experience Manager  {#upload-and-add-new-assets-to-aem}

Gli utenti possono aggiungere nuove risorse all&#39;archivio DAM. Ad esempio, potete essere un fotografo di agenzia o un appaltatore che desidera aggiungere un gran numero di foto da una fotografia al repository del Experience Manager . Per aggiungere nuovo contenuto al  Experience Manager, fate clic su ![Carica nell&#39;icona cloud](assets/do-not-localize/upload_to_cloud_da2.png) nella barra superiore dell&#39;app. Individuate i file di risorse nel file system locale e fate clic su **[!UICONTROL Select]**. L&#39;app avvia il caricamento della risorsa e visualizza una barra di avanzamento in basso se il caricamento della risorsa richiede più tempo. Non utilizzate spazi bianchi e caratteri non validi durante la creazione o il caricamento di cartelle. Visualizzare un elenco di caratteri in [Crea cartelle in [!DNL Assets]](https://experienceleague.adobe.com/docs/experience-manager-65/assets/managing/manage-assets.html#creating-folders).
...
>[!NOTE]
>
>Se i file non vengono caricati e se vi state connettendo  Experience Manager 6.5.1 o una distribuzione successiva, consultate questa sezione [informazioni sulla risoluzione dei problemi](troubleshoot.md#upload-fails).
...
>[!NOTE]
>
>Non utilizzate la funzionalità di caricamento per migrare le risorse tra due distribuzioni  Experience Manager. Vedete la [guida alla migrazione](https://experienceleague.adobe.com/docs/experience-manager-65/assets/administer/assets-migration-guide.html).

### Elenco delle attività trasferite {#list-of-transferred-assets}

Per visualizzare l&#39;elenco delle risorse trasferite in una determinata sessione, consultate [Caricare le risorse  Experience Manager](#upload-and-add-new-assets-to-aem).

## Flusso di lavoro avanzato: inizia dall&#39;interfaccia [!DNL Assets] Web {#adv-workflow-start-from-aem-ui}

Se necessario, avviate il flusso di lavoro dall’interfaccia Web Risorse. L&#39;app desktop si integra con il Experience Manager  da sostituire quando richiesto tramite Azioni desktop.
...
![Azioni desktop nell&#39;](assets/assets_web_actions_da2.png "interfaccia Web del Experience Manager Azioni desktop nell&#39;interfaccia Web del Experience Manager ")
...
* Caricate le modifiche locali  Experience Manager.
...
Se una risorsa scaricata localmente viene aggiornata sul server di Experience Manager , l&#39;app visualizza uno stato **[!UICONTROL Modified remotely]**. Per rimuovere la copia locale o aggiornare la copia locale, fare clic rispettivamente su [!UICONTROL Remove] o [!UICONTROL Update]. I collegamenti presenti nella finestra di dialogo consentono di visualizzare entrambe le versioni della risorsa.
...
1. Tenete a portata di mano un file INDD con le risorse inserite in  Experience Manager. Per informazioni su come creare tale file INDD, vedere [Inserimento di elementi grafici](https://helpx.adobe.com/indesign/using/placing-graphics.html).
1. Dall&#39;interno dell&#39;app desktop, **[!UICONTROL Edit]** il file INDD con le risorse inserite in  Experience Manager.
...
1. Una volta completate le modifiche nel documento InDesign , salvatelo e caricatelo  Experience Manager utilizzando l&#39;app desktop.

## Flusso di lavoro avanzato: scaricare le risorse localmente {#adv-workflow-download-assets-locally}

L&#39;app scarica le risorse da  server di Experience Manager localmente sul file system in molti scenari. I download utilizzano larghezza di banda e spazio su disco. La conoscenza degli scenari consente di ottimizzare il tempo di attesa per il completamento dei download.
...
Quando utilizzate l&#39;azione [!UICONTROL Edit] per modificare una risorsa in un&#39;applicazione desktop nativa, la risorsa viene scaricata localmente se non è già disponibile localmente. Consultate [Modificare le risorse e caricare le risorse aggiornate  Experience Manager](#edit-assets-upload-updated-assets).

Se l&#39;app è installata e consentita, completa le azioni quando utilizzi [!UICONTROL Desktop Actions] dall&#39;interfaccia Web  Experience Manager. L&#39;app scarica prima la risorsa e quindi completa l&#39;azione.
