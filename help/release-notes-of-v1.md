...
* **Versione 1.5**: stabilità e resilienza delle applicazioni rispetto a vari problemi di rete, migliore supporto.
* **Versione 1.4**: la possibilità di caricare cartelle gerarchiche in background con il monitoraggio dell’avanzamento.
* **Versione 1.3**: miglioramenti in termini di prestazioni e stabilità dell’accesso ai file e del salvataggio delle modifiche in AEM, soprattutto dalle applicazioni desktop Creative Cloud, come InDesign, Illustrator o Photoshop. L'obiettivo era quello di offrire agli utenti un'esperienza desktop più locale quando lavorano con i file, gestendo allo stesso tempo le operazioni di trasferimento dei dati di rete in background.
...
L’app desktop AEM 1.9 si è concentrata sul miglioramento dell’esperienza utente in caso di caricamenti di grandi dimensioni, sulle operazioni in background e sull’esperienza ottimizzata quando si aprono risorse con file collegati (come InDesign).
...
* Installazione e disinstallazione:
   ...
   * (Mac) Script manuale per rimuovere completamente i file delle app desktop (CQ-4216489)
...
* I file vengono salvati in un percorso dell'unità di rete montato tramite l'app desktop in due fasi.
* Nella prima fase, un file viene salvato localmente. In questo modo, l’utente che salva il file non deve attendere il completo trasferimento del file in AEM e può riprendere a lavorare non appena viene salvato.
...
**Tieni disponibile la sessione AEM.** L’app desktop AEM dipende da una sessione aperta al server Risorse AEM per garantire il corretto funzionamento. Per gli utenti che lavorano con l’app desktop ogni giorno, si consiglia di disinstallare Risorse AEM alla fine della giornata per forzare la disconnessione, e quindi di installare Risorse AEM la mattina per verificare che siano connessi e che la condivisione di rete sia operativa.
...
* Integrazione con il Finder:
   * Quando si trascinano o si trascinano file di grandi dimensioni, è possibile che si verifichi un timeout durante il trasferimento dei file in background. Questo si traduce in un `Error - 36`. La soluzione alternativa consiste nel trascinare o aprire nuovamente la risorsa (CQ-4219628)
...
* [Documentazione di AEM](https://helpx.adobe.com/support/experience-manager/6-4.html)
