...
L’Adobe consiglia vivamente di utilizzare AEM’ultima versione dell’app desktop per usufruire delle funzionalità più recenti, delle correzioni di bug più recenti e delle migliori prestazioni possibili.

La versione dell’app desktop AEM che intendi installare nel computer locale richiede una versione specifica AEM server/componenti aggiuntivi lato server (Service Pack, hotfix o feature pack). Assicurati che il server AEM sia configurato correttamente prima di connetterti ad esso per la prima volta. Se hai bisogno di assistenza, contatta l’amministratore AEM.
...
AEM’app desktop 1.10 è incentrata sul miglioramento dell’esperienza utente in merito a caricamenti di grandi dimensioni, alle informazioni sulle operazioni in background e all’esperienza ottimizzata durante l’apertura di risorse con file collegati (come InDesign).
...
Per istruzioni dettagliate, consulta [Installare e configurare AEM’app desktop](install-configure-app-v1.md).
...
AEM’app desktop 1.9 si è concentrata sul miglioramento dell’esperienza utente in merito a caricamenti di grandi dimensioni, alle informazioni sulle operazioni in background e all’esperienza ottimizzata durante l’apertura di risorse con file collegati (come InDesign).
...
* Quando si scaricano i documenti di InDesign, comprese le risorse collegate memorizzate in AEM Assets, l’app desktop scarica prima tutte le risorse collegate, prima di aprire il documento [!UICONTROL Adobe InDesign] e indica il download delle risorse collegate. Ad esempio, 5 su 24.

[!UICONTROL Bulk Uploads]

Il caricamento di gerarchie di cartelle di grandi dimensioni tramite [!UICONTROL Create] > [!UICONTROL Upload Folder] nell’interfaccia utente AEM Web oppure la copia e la selezione di &quot;Incolla risorse&quot; nel Finder o in Esplora risorse nel menu di scelta rapida dell’app desktop attiverà l’utilizzo di questa finestra di dialogo.
...
L’icona dell’applicazione indica lo stato delle operazioni in background per fornire un migliore segnale visivo agli utenti. Ad esempio, quando l’applicazione non è connessa a AEM l’icona verrà disattivata, quando c’è un caricamento attivo verrà visualizzata una sovrapposizione &quot;sincrona&quot;, ecc.

**Recupero preliminare delle risorse collegate**

Per migliorare l’esperienza utente quando si lavora con documenti InDesign che includono risorse collegate memorizzate in AEM, l’app desktop tenterà di preacquisire questi file collegati nella cache locale prima di scaricare e aprire il documento InDesign. In questo modo l’utente avrà i file collegati disponibili localmente e non dovrà aspettare più a lungo quando accederà a tali file in InDesign (nel pannello Collegamenti).
...
AEM l’app desktop 1.8.1 è stata migliorata con una versione che segue rapidamente, quando si aprono più file contemporaneamente dall’interfaccia utente AEM alla versione 1.8 (CQ-4237747, CQ-4238780). I miglioramenti nell’app desktop AEM 1.8 sono:

* Memorizzazione in cache: nuova interfaccia utente per la gestione AEM cache delle app desktop (CQ-4208690), tra cui:
...
* Accesso: (Win) È stato corretto l&#39;accesso a AEM&#39;istanza configurata per l&#39;utilizzo di SAML e SSL (CQ-4216353)

* Rete:

   * alla scadenza di una sessione di AEM, l’utente ora riceve una notifica e può fare clic sulla notifica per effettuare nuovamente l’accesso (CQ-4202028).

   * (Mac) Aggiungi il supporto per la connessione a AEM tramite la configurazione proxy .pac (CQ-4233430).
...
* Memorizzazione in cache:
   ...
   * Correggi gli errori del disco e i problemi di apertura durante l&#39;apertura dei file in Photoshop, InDesign, Finder (CQ-4223993, CQ-4217603, CQ-4223717).
...
>[!NOTE]
>
>I problemi rilevati nei carichi AEM delle versioni beta dell’app desktop 1.7 (che non erano presenti nella versione 1.6 non sono riportati nelle note sulla versione).
...
* È stato migliorato il processo di accesso per AEM:
...
* Problemi selezionati rilevati e risolti nella versione 1.6 Beta:
   ...
   * Errore durante il caricamento su AEM (CQ-4217006) durante il trascinamento dei file.
...
* Nuove funzioni e miglioramenti: Aggiungi la funzionalità Copia/Incolla all&#39;integrazione del Finder per consentire il trasferimento diretto da Desktop a AEM (CQ-4208158).
...
Nota sugli avvisi sui collegamenti: Le applicazioni di Creative Cloud (come InDesign) acquisiscono uno &quot;snapshot&quot; dell&#39;ora dell&#39;ultima modifica dell&#39;elemento al momento in cui viene posizionato. Se tale data cambia in un momento successivo, l’app Adobe Creative Cloud segnala che i collegamenti sono obsoleti. Questo è riportato in un paio di modi:

* Quando l’app Adobe Creative Cloud viene avviata, viene visualizzata una finestra di dialogo per informare l’utente che le risorse collegate sono obsolete e richiedere all’utente di intervenire.
...
Questo comportamento è lo stesso per le risorse sul disco locale e le risorse in una directory montata AEM Desktop, con le seguenti eccezioni:
...
* Se un utente modifica una risorsa inserita tramite la directory montata AEM desktop e quindi cancella la cache locale, la risorsa inserita verrà segnalata come obsoleta.
...
**Versione 1.5.0.x per Mac OS X e Windows:** Questa versione dell’app desktop AEM fornisce i seguenti vantaggi:
...
* Altre correzioni:
   ...
   * Gestione migliore degli URL AEM Assets nelle preferenze (CQ-97388).
...
* Caricamento semplificato di cartelle gerarchiche tramite la nuova azione Crea > Carica cartella nell’interfaccia utente touch.
   ...
   * L’app desktop analizza la gerarchia di cartelle specificata sul desktop in background e carica i file in AEM Assets
...
* Migliore gestione delle applicazioni desktop che utilizzano operazioni di file intermedi per salvare un file (salvataggi parziali e file temporanei), che consente alla timeline AEM Assets di visualizzare le informazioni corrette sulla versione e sul caricamento delle risorse.
...
Poiché AEM’app desktop deve essere eseguita per gli utenti che non dispongono dei diritti di amministratore sul computer locale, il punto di montaggio per l’archivio AEM Assets è stato modificato in 1.4 e 1.5 in una sottocartella DAM nella cartella locale dell’utente su MacOS (CQ-104183).

Poiché la cartella /Volumes non richiede più i diritti amministrativi, questa modifica è stata ripristinata in 1.5.1. Questo consente anche di condividere documenti InDesign che hanno inserito AEM risorse tra gli utenti MacOS.
...
* Per entrambe le piattaforme (Windows e Mac):
   * Il protocollo per accedere/scaricare le risorse e caricare le modifiche in AEM è stato modificato nel protocollo AEM nativo, che è un protocollo RESTful basato su HTTP. Offre un maggiore controllo sulle operazioni di rete ed è più compatibile con l&#39;infrastruttura di rete.

>[!NOTE]
>
>In Mac OS X, la modifica del protocollo dell&#39;unità di rete locale da WebDAV a SMB1 comporta un percorso locale diverso per la stessa risorsa nel repository. Questo potrebbe influire sui collegamenti ai file inseriti nelle applicazioni Adobe Creative Cloud tramite il comando &quot;Place&quot;. Per ulteriori informazioni, consulta l’ [Usa app desktop AEM](use-app-v1.md) .
...
* Nella prima fase, un file viene salvato localmente. In questo modo, l&#39;utente che salva il file non deve attendere fino a quando il file non viene completamente trasferito in AEM e può riprendere a funzionare non appena il file viene salvato.
* Nella seconda fase, l’app desktop carica un file aggiornato AEM server dopo un ritardo predefinito (ad esempio, 30 ). Questa operazione si verifica in background. Utilizza l&#39;opzione **Mostra stato sincronizzazione file in background** per visualizzare lo stato dell&#39;operazione di caricamento.

## Avvisi importanti {#important-notices}

**Caricamento cartella.** Si consiglia di utilizzare la nuova funzionalità di caricamento cartelle per caricare cartelle gerarchiche più grandi in AEM invece di utilizzare una copia / trascinamento e rilasciare in un archivio AEM montato a livello di Finder / Explorer. Quando utilizzi la funzione di caricamento cartelle, l’app desktop comunica direttamente con AEM e quindi ha un controllo molto migliore sul processo complessivo.

**Mantieni AEM sessione disponibile.** AEM’app desktop dipende da una sessione aperta al server AEM Assets per garantire il corretto funzionamento. Per gli utenti che lavorano quotidianamente con l’app desktop, si consiglia di smontare AEM Assets alla fine della giornata per forzare la disconnessione, e poi di installare AEM Assets la mattina per assicurarsi che siano connessi e che la condivisione di rete sia operativa.
...
**Utilizzare certificati SSL con firma CA.** Tieni presente che AEM’app desktop non supporta certificati SSL autofirmati quando ci si connette a AEM tramite una connessione protetta HTTPS. Per tali connessioni è necessario un certificato con firma CA sul server. (CQ-87941)

## Problemi noti {#known-issues}

* Generale:
   * Gli URL del server devono puntare al server senza un percorso (ad esempio, `http://server`, `https://server`, `http://server:port` o `https://server:port`). I percorsi di contesto e le sottocartelle diverse da /content/dam non sono supportati (CQ-89343, CQ-87272)
* Nomi di file / localizzazione:
   ...
   * Alcune applicazioni, come Adobe Illustrator, potrebbero creare file con nomi non supportati in AEM. Ad esempio, l’aggiunta di `Converted` dopo la conversione di un file, che lo impedisce di essere caricato (CQ-4216985)
...
* [Usare AEM’app desktop v1.x](use-app-v1.md)
